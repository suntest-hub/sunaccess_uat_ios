...
WL.JSONStore = (function(_) {
  ...
  _.each(publicAPI, function(apiName) {
   ...
   stub['syncOptions'] = { SYNC_NONE : 2, SYNC_UPSTREAM : 1, SYNC_DOWNSTREAM : 0 };
  return stub;
}(WL_));
...
/*globals WLJQ, WL_, WLResourceRequest, cordova, sjcl, setTimeout, document, localStorage, JSONStoreWin8, ArrayBuffer */
...
WL.constant = (function() {
  ...
  return {
    ...
    OS_SECURITY_FAILURE : OS_SECURITY_FAILURE,
    ...
    DESTROY_FAILED_METADATA_REMOVAL_FAILURE : DESTROY_FAILED_METADATA_REMOVAL_FAILURE,
  ...
  var check = WL.check,
    ...
    __inequalityComparator = function(value, index, list) {
      var self = this,
        ...
        pattRight,
        hasOwn = Object.prototype.hasOwnProperty;
      _.each(list, function(iSearch, vSearch) {
...
if (WL.Client.getEnvironment() === WL.Environment.IPHONE ||
  ...
  WL.Client.getEnvironment() === WL.Environment.WINDOWS ||
  WL.Client.getEnvironment() === WL.Environment.WINDOWSPHONE8) {
  ...
  **/
  WL.db = (function(jQuery) {
    ...
    var cdv = cordova,
      ...
      _sync = function(collection,options){
        __callNative([collection], options, STORAGE_PLUGIN, SYNC_METHOD);
      },
      ...
      _destroy = function(options, username) {
  ...
  WL.storage = (function(jQuery) {
    ...
    var ls = localStorage,
      ...
      _destroy = function(store) {
        ...
        setTimeout(function () {
          /*jshint forin:false*/
          validKeys = Object.keys(ls);
          for(var i = 0; i < validKeys.length; i++) {
            metaKey = validKeys[i];
            metaIndex = metaKey.indexOf(constant.METADATA_TAG);
            patt = new RegExp('"' + constant.USERNAME + '":', ['g']);
            meta = ls.getItem(metaKey);
              if(!check.isUndefined(meta) && meta.match(patt) !== null && metaIndex > -1) {
                metaKeys.push(metaKey);
  ...
  WL.browser = (function(jQuery, underscore) {
    ...
    var store = new Store(),
      ...
      __queryProcessor = function(ctx) {
        ...
        _.each(ctx.filter, function(value, index) {
...
WL._JSONStoreImpl = (function(jQuery, underscore) {
  ...
  var db = WL.db || WL.browser,
    ...
    JSONStoreInstance = function(name, searchFields, adapter, additionalSearchFields, username, deferred) {
      ...
      */
      /* jshint shadow:true */
      /*jshint maxparams:8*/
     JSONStoreInstance = function(name, searchFields, adapter, additionalSearchFields, username, deferred, onSyncSuccess, onSyncFailure){
      ...
      this.additionalSearchFields = additionalSearchFields || {};
      this.promise = deferred.promise();
      this.onSyncSuccess = onSyncSuccess|| function(){};
      this.onSyncFailure = onSyncFailure|| function(){};
    },
    ...
    */
      ...
      var deferred = $.Deferred(),
        ...
        allDirtySuccess = function(data) {
          ...
          for (i = 0; i < len; i++) {
            ...
            arr.push(inst.sendResourceRequest({
    ...
    /**
      ...
      @param [options] {Options}
      */
    __store = function(name, username, searchFields, additionalSearchFields, data, options, onSyncSuccess, onSyncFailure){
      ...
      var deferred = $.Deferred(),
        ...
        syncActionReceiverName = '__WL_syncActionReceiverFor'+name;

        callbacks.isAdd = options.isAdd;
	function actionReceiverForSyncAdd(received){
            if(received.data.success === 'true')
            {
                onSyncSuccess(received.data.message);
            }
            else if(received.data.success === 'false')
            {
                onSyncFailure(received.data.message);
            }
            else
            {
                onSyncFailure('Sync failed in native layer');
            }
            WL.App.removeActionReceiver(syncActionReceiverName);
          }
         
         if(onSyncSuccess !== undefined && onSyncFailure !== undefined)
         {
               WL.App.addActionReceiver(syncActionReceiverName, actionReceiverForSyncAdd);
         }
    ...
    /**
      ...
      @param [options] {Options}
      */
    __replace = function(name, username, doc, options,onSyncSuccess,onSyncFailure) {
      ...
      function actionReceiverForSyncReplace(received){
            if(received.data.success === 'true')
            {
                onSyncSuccess(received.data.message);
            }
         ...
         if(onSyncSuccess !== undefined && onSyncFailure !== undefined)
         {
               WL.App.addActionReceiver(syncActionReceiverName, actionReceiverForSyncReplace);
         }
    ...
    /**
      ...
      @param [options] {Options}
      */
    __remove = function(name, username, searchFields, additionalSearchFields, doc, options,onSyncSuccess,onSyncFailure) {
      ...
      var deferred = $.Deferred(),
        ...
        function actionReceiverForSyncRemove(received){
            if(received.data.success === 'true')
            {
                onSyncSuccess(received.data.message);
            }
         ...
         if(onSyncSuccess !== undefined && onSyncFailure !== undefined)
         {
               WL.App.addActionReceiver(syncActionReceiverName, actionReceiverForSyncRemove);
         }
    ...
    __handleOptions = function(options, searchFields, additionalSearchFields) {
      ...
      if (check.isObject(options)) {
        ...
        if(check.isArray(options.filter)){
          _.each(options.filter, function(value) {
            var newValue = value;
    ...
    /**
      ...
      */
    _destroy = function(optionsOrUsername) {
      ...
      //An options object was passed, probably something like: {onSuccess: func, onFailure: func}
    ...
    _init = function(collections, options) {
      ...
      var arrp = [],
        ...
        setOptions = function(key) {
          ...
          if(collections[key].sync !== undefined){
            if(collections[key].sync.syncPolicy !== undefined){
               options.syncPolicy = collections[key].sync.syncPolicy;
            }
            else{
               options.syncPolicy = 2;//2 means SYNC_NONE
            }
            options.syncAdapterPath = collections[key].sync.syncAdapterPath || '';
            options.onSyncSuccess = collections[key].sync.onSyncSuccess || function(){};
            options.onSyncFailure = collections[key].sync.onSyncFailure || function(){};
         }
        },
	...
	if (check.isBoolean(options.localKeyGen)) {
        ...
        if (check.isNumber(options.syncPolicy)) {
      ...
      if(check.isObject(options) && !check.isUndefined(options.pbkdf2Iterations) && check.isInt(options.pbkdf2Iterations)){
        collectionPBKDF2Iterations = options.pbkdf2Iterations;
      }

      if(check.isObject(options) && !check.isUndefined(options.syncAdapterPath) && check.isString(options.syncAdapterPath)){
        syncAdapterPath = options.syncAdapterPath;
      }

      if(check.isObject(options) && !check.isUndefined(options.syncPolicy) && check.isInt(options.syncPolicy)){
        syncPolicy = options.syncPolicy;
      }

      if(syncPolicy!== 2){
        instance = new JSONStoreInstance(collectionName, collectionsearchFields, collectionAdapter,
                   collectionAdditionalSearchFields, username, deferred, options.onSyncSuccess, options.onSyncFailure);
      }else{
        instance = new JSONStoreInstance(collectionName, collectionsearchFields, collectionAdapter,
                   collectionAdditionalSearchFields, username, deferred);
      }

	//wi 116806 - Adding syncpolicy in options to accommodate couchdb-jsonstore sync
      collectionOptions = {
        ...
        syncAdapterPath: syncAdapterPath,
        syncActionReceiverName: syncActionReceiverName
      };
	
	//wi 116664 - Adding action receiver for sync result of couchdb-jsonstore sync if the syncPolicy is set to SYNC_DONWSTREAM
	//Sends message from native to appropriate callback and removes the previously registered action receiver.
        // jshint unused:false
        function actionReceiverForSyncInit(received){
          try{
		if(received.data.success === 'true')
		{
			options.onSyncSuccess(received.data.message);
		}
            else if(received.data.success === 'false')
            {
              options.onSyncFailure(received.data.message);
            }
            else
            {
              options.onSyncFailure('Sync failed in native layer');
            }
            WL.App.removeActionReceiver(syncActionReceiverName);
          }catch(error){
             console.log('Error occurred in action receiver for sync : ' + JSON.stringify(error));
          }
        }
          //wi 116664 - If sync policy is not SYNC_NONE, adding an action receiver
          if(syncPolicy !== 2){
		try{
			WL.App.addActionReceiver(syncActionReceiverName, actionReceiverForSyncInit);
		}catch(error){
		console.log('Exception occurred while adding action receiver for sync : ' + JSON.stringify(error));
            }
          }
  ...
  PushInstance.prototype = {
    ...
    /**
      ...
      @private
      */
    sendResourceRequest: function(options) {
      ...
      if(invocationData && invocationData.parameters){
  ...
  JSONStoreInstance.prototype = {
    ...
    /**
        Syncs with the remote Cloudant DB based on sync policy set while initializing the collection
    */
    sync: function() {
      var options = {},
          that = this,
          deferred = $.Deferred(),
          usr = this.username || '',
          callbacks = __generateCallbacks(options, 'sync', this.name, usr, deferred),
          syncActionReceiverName = '__WL_syncActionReceiverFor'+this.name;

          function actionReceiverForSync(received){
              if(received.data.success === 'true'){
                  that.onSyncSuccess(received.data.message);
              }
              else if(received.data.success === 'false'){
                 that.onSyncFailure(received.data.message);
              }
              else{
                 that.onSyncFailure('Sync failed in native layer');
              }
              WL.App.removeActionReceiver(syncActionReceiverName);
          }

          if(this.onSyncSuccess !== undefined && this.onSyncFailure !== undefined){
              WL.App.addActionReceiver(syncActionReceiverName, actionReceiverForSync);
          }

         db.sync(this.name, callbacks);
         return deferred.promise();
    },
    ...
    store: function(data, options) {
      ...
      return __store(this.name, this.username, this.searchFields, this.additionalSearchFields, data, options,this.onSyncSuccess, this.onSyncFailure);
    ...
    replace: function(doc, options) {
      ...
      return __replace(this.name, this.username, doc, options,this.onSyncSuccess,this.onSyncFailure);
    },
    ...
    remove: function(doc, options) {
      ...
      return __remove(this.name, this.username, this.searchFields, this.additionalSearchFields, doc, options,this.onSyncSuccess,this.onSyncFailure);
    ...
    change: function(data, options) {
      ...
      if(check.isArray(options.replaceCriteria)) {
        _.each(options.replaceCriteria, function(value) {
          var newValue = value;
    ...
    load: function(options) {
      ...
      var deferred = $.Deferred(),
        ...
        myKey,
        myPath,
        resourceRequestSuccessCallback = function(response) {
          ...
          if (input.length > 1) {
            ...
            resultData = [response.responseJSON[collectionAdapter.load.key]];
          }
        ...
        resourceRequestFailureCallback = function(data) {
      ...
      if (check.isValidAdapter(collectionAdapter) &&
        ...
        var resourceRequest = new WLResourceRequest('adapters/' + invocationData.adapter + '/' + invocationData.procedure,  WLResourceRequest.GET);
        resourceRequest.send().then(
          resourceRequestSuccessCallback,
          resourceRequestFailureCallback
        );
