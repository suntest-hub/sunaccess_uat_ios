...
typedef void (^WLCompletionHandler)(bool response, NSString* error);
...
/* Private. The syncpolicy for this collection
 @private
 */
@property(nonatomic) JSONStoreSyncPolicy _syncPolicy;


/* Private. The syncAdapterPath for this collection
 @private
 */
@property(nonatomic) NSString* _syncAdapterPath;

/* Private. The syncAdapterPath for this collection
 @private
 */
@property (nonatomic) WLCompletionHandler _completionHandler;
...
/**
 Fetches documents from cloud db from db with name same as the collection and adds to the collection. Provides a completion handler to notify status of the sync.
 @return void
 @since IBM Worklight V8.0
 @private
 */
-(void) syncDownstream;

/**
 Performs an upstream, downstream or no sync based on the sync policy set during initialization of the collection. Provides a completion handler to notify status of the sync.
 @return void
 @since IBM Worklight V8.0
 @private
 */
