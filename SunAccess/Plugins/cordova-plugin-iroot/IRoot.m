...
- (void) isRooted:(CDVInvokedUrlCommand*)command;
{
    ...
    //    BOOL Ptraceflag = [[[NSBundle mainBundle] objectForInfoDictionaryKey:@"PtraceFlag"] boolValue];
    
    @try
    {
        bool _valAntiHooking = NO;
        bool _sandbox_integrity_compromised = NO;
        
#if !(TARGET_IPHONE_SIMULATOR)
        [self denyPtrace];
        _valAntiHooking = [self checkAntiHooking];
        _sandbox_integrity_compromised = [self sandbox_integrity_compromised];
        
#endif
        
        bool _roocheck = NO;
        bool jailbroken = [self jailbroken];
        bool _dyld_check = [self dyld_check];
        bool _SSHportCheck = [self isSSHPortOpen];
        
        if(jailbroken == YES || _dyld_check == true || _sandbox_integrity_compromised == true || _valAntiHooking == true || _SSHportCheck == true){
            _roocheck = YES;
        }
        
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsBool:_roocheck];
    }
...
-(bool) checkAntiHooking
{
    void * funcptr;
    unsigned int * funcddr = (unsigned int * ) funcptr;
    if(funcptr){
        if(funcddr[0] == 0xe51ff004)
            return true;
    }
    return false;
}
...
- (bool) jailbroken {
    ...
    else if ([[[NSBundle mainBundle] infoDictionary] objectForKey:@"SignerIdentity"] !=nil)
    {
        return YES;
    }
